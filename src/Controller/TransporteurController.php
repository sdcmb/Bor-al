...
use \Emc\Quotation;//importation de la classe quotation de Boxtal
use \Emc\ListPoints;//importation de la classe listePoints de Boxtal
...
class TransporteurController extends AbstractController
{
    ...
    //country, zipcode, city, adress, collection_date, colis_valeur, colis_poids
    {
        ...
        if($request->request->count()>0) {
            $cotation = new Session();//création d'une nouvelle session pour utliser des variables de sessions
            ...
            $cotation->set('zipcode',$request->request->get('zipcode'));//même chose..
    ...
    switch ($dest) {
        case 'client1':
            $to = array(//tableau contenant l'adresse de réception du colis
                "country" => $cotation->get('country'), //country prend comme valeur, la valeur country entré dans le formulaire info livraison
                // "state" => "", if required, state must be an ISO code as well
    ...
    $additionalParams = array(//paramètre supplémentaire du colis
        'collection_date' => $cotation->get('collection_date'),//collection_date prend comme valeur, la valeur collection_date entré dans le formulaire info livraison
        'delay' => 'aucun',
    ...
    */
    ...
    $parcels = array(//dimension, poids du colis
        ...
        'dimensions' => array(
            1 => array(
                'poids' => $cotation->get('colis_poids'), // //poids prend comme valeur, la valeur colis_poids entré dans le formulaire info livraison
    ...
    $lib->getQuotation($from, $to, $parcels, $additionalParams);
        ...
        return $this->render('transporteur/cotation.html.twig', [
            'controller_name' => 'TransporteurController',
            'cotations' => $lib->offers,//retourne les offres
            'curl_error' => $lib->curl_error,//retourne les erreurs
            'resp_error' => $lib->resp_error//retourne les erreurs
        ]);
    }
    ...
    public function pointRelais(Request $request)//renvoit des point relais en fonction des paramètres transporteurName, country, zpidcode, city
    {
        ...
        $params = array(
            ...
            'pays' => $session->get('country'), //pays prend comme valeur, la valeur country entré dans le formulaire info livraison
        ...
        return $this->render('transporteur/listePointRelais.html.twig', [
            'controller_name' => 'TransporteurController',
            'carriers' => $lib->list_points,//retourne la liste des points relais
            'curl_error' => $lib->curl_error,//retourne les erreurs
            'resp_error' => $lib->resp_error//retourne les erreurs
